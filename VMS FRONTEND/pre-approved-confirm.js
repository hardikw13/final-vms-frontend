const FALLBACK_DATA = {
  ...
  details: [
    ...
    { label: "Host", value: "Dr. Anjali Mehta — Dept. of CS" },
    { label: "Purpose", value: "Research Collaboration Meeting" },
    { label: "Date", value: "Today, 21 Jun 2026" },
    { label: "Slot", value: "2:00 PM – 4:00 PM" }
  ],
...
async function loadData() {
  try {
    const res = await fetch("pre-approved-confirm-data.json");
    if (!res.ok) throw new Error("bad response");
    return await res.json();
  } catch (err) {
    console.warn("Falling back to inline data (serve via a local server to load pre-approved-confirm-data.json):", err.message);
...
(async function init() {
  ...
  document.getElementById("verifiedTitle").textContent = data.verified.title;
  document.getElementById("invitationId").textContent = data.verified.invitationId;
  document.getElementById("detailsTitle").textContent = data.detailsTitle;

  const list = document.getElementById("detailsList");
  data.details.forEach((item) => list.appendChild(renderDetailRow(item)));
  ...
  rescanLink.textContent = data.rescanLinkText;
  rescanLink.href = data.rescanLink;
